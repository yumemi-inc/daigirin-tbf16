...
データストリームを扱う場合、イベント発生のタイミングが不確定なので、イベント監視に「待ち」が発生します。その監視をメインスレッドで行うと、画面応答やアプリ機能のパフォーマンスは低下します。そのため、イベントの監視とイベントに対する処理を分離し、非同期処理を通じて効率的に管理します。別スレッドで監視することでパフォーマンス低下を防ぎます。
...
本記事のスコープは、データストリームから発生したデータを制御クラスが発行し、そのデータを受け取って処理を実行させるまでの過程です。その前後のデータストリームの実装や特性、購読後の処理実行は取り扱いしません。

なお、これから紹介するコード例は簡易的に示します。実際に利用する場合は、クラス内で適切に定義する、非同期関数を適切に実行させる必要があります。また、同様な機能をもつ関数がある場合もありますが、それらは省略します。ご了承ください。
